...
from tests.factories import CategoryFactory, TodoFactory, UserFactory
...
class TestCategoryModel:
    ...
    def test_category_belongs_to_user(self):
        """Cada categoria debe estar asociada a un usuario"""
        user = UserFactory(username="julio")
        category = CategoryFactory(created_by=user)

        assert category.created_by.username == "julio"

    def test_category_ordering(self):
        """Probar que el order del modelo funcione correctamente"""
        user = UserFactory()
...
class TestTodoModel:
    ...
    def test_todo_belongs_user(self):
        user = UserFactory(username="julioz")
        todo = TodoFactory(user=user)

        assert todo.user.username == "julioz"
